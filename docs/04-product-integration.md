...
flowchart TD
    A["每周/双周跑分<br/>~30 min"]
    B["生成 HTML 报告<br/>按 Coach 分组"]
    C["Coach 团队复核<br/>HIGH → 加投入<br/>MEDIUM → 测新模板<br/>LOW → 1-on-1 沟通<br/>DROP RISK → 复核确认"]
    D["达人池状态更新<br/>→ CRM 系统"]
...
报告里达人**按"所属 Coach"分组**，组内**按潜力评分倒序**——让每位 Coach 一打开报告就能看到自己下属的完整画面，而不是混在 600+ 数据里找。
...
- **跑分频率**：每周/双周 1 次
- **服务规模**：600+ 大使
- **辅助清退**：基于 Spring Semester 数据 ~400 位
